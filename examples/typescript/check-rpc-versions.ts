...
import { NearRpcClient, status, block, experimentalChangesInBlock, experimentalGenesisConfig } from '@near-js/jsonrpc-client';

const endpoints = [
  { name: 'Mainnet FastNEAR (archival)', url: 'https://archival-rpc.mainnet.fastnear.com' },
  ...
  { name: 'Testnet FastNEAR (archival)', url: 'https://archival-rpc.testnet.fastnear.com' },
...
for (const endpoint of endpoints) {
  ...
  if (statusResponse.syncInfo.earliestBlockHeight) {
    console.log(`Earliest block: ${statusResponse.syncInfo.earliestBlockHeight}`);
  }
  ...
  try {
    ...
    if (error.data?.name === 'UNKNOWN_BLOCK' || error.message?.includes('UNKNOWN_BLOCK')) {
  ...
  try {
    const changesInBlock = await experimentalChangesInBlock(client, { blockId: genesisHeight });
    ...
    if (changesInBlock.changes) {
      ...
      if (changesInBlock.changes.length > 0) {
        ...
        console.log(`  - Sample change: ${sampleChange.type} for ${sampleChange.accountId}`);
      }
    } else {
    ...
    if (error.data?.name === 'UNKNOWN_BLOCK' || error.message?.includes('UNKNOWN_BLOCK')) {
      isArchival = false;
      console.log(`Can access genesis+100 block changes: ❌ No (UNKNOWN_BLOCK)`);
    } else {
  ...
  try {
    const recentChanges = await experimentalChangesInBlock(client, { finality: 'final' });
    ...
    if (recentChanges.changes) {
      for (const change of recentChanges.changes) {
        recentChangeTypes[change.type] = (recentChangeTypes[change.type] || 0) + 1;
      }
      console.log(`  - Total changes: ${recentChanges.changes.length}`);
      if (Object.keys(recentChangeTypes).length > 0) {
        console.log(`  - Change types: ${Object.keys(recentChangeTypes).join(', ')}`);
      }
    } else {
      console.log(`  - No changes data`);
    }
