...
describe('Generator Integration Tests', () => {
  ...
  describe('Regression Detection', () => {
    it('should detect if API spec has changed significantly', async () => {
      ...
      console.log(
        `Current API stats: ${pathCount} paths, ${schemaCount} schemas`
      );
    ...
    it('should handle new methods gracefully', async () => {
      ...
      if (blockPath) {
        ...
        expect(Object.keys(modifiedSpec.paths).length).toBeGreaterThan(
          Object.keys(specObject.paths).length
        );
      } else {
  ...
  describe('Performance Tests', () => {
    ...
    it('should generate reasonable file sizes', async () => {
      ...
      const typesFile = join(
        projectRoot,
        'packages/jsonrpc-types/src/types.ts'
      );
      const schemasFile = join(
        projectRoot,
        'packages/jsonrpc-types/src/schemas.ts'
      );
      ...
      console.log(
        `File sizes: types=${typesStats.size}, schemas=${schemasStats.size}, client=${clientStats.size}`
      );
