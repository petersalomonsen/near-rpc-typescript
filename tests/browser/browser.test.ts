...
test.describe('NEAR RPC Client Browser Tests', () => {
  test('should load the test page successfully', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('NEAR RPC Client Browser Test');
  ...
  test('should successfully get latest block', async ({ page }) => {
    ...
    await expect(page.locator('#latestBlockResult')).toHaveClass(/success/, { timeout: 10000 });
  ...
  test('should successfully view account', async ({ page }) => {
    ...
    // Wait for the result to appear and check it's successful
    await expect(page.locator('#accountViewResult')).toHaveClass(/success/, { timeout: 10000 });
  ...
  test('should successfully get network status', async ({ page }) => {
    ...
    // Wait for the result to appear and check it's successful
    await expect(page.locator('#networkStatusResult')).toHaveClass(/success/, { timeout: 10000 });
  ...
  test('should successfully get gas price', async ({ page }) => {
    ...
    // Wait for the result to appear and check it's successful
    await expect(page.locator('#gasPriceResult')).toHaveClass(/success/, { timeout: 10000 });
  ...
  test('should successfully get specific block', async ({ page }) => {
    ...
    // Wait for the result to appear and check it's successful
    await expect(page.locator('#specificBlockResult')).toHaveClass(/success/, { timeout: 10000 });
  ...
  test('should run all tests successfully', async ({ page }) => {
    ...
    await expect(page.locator('#runAllTests')).toContainText('passed', { timeout: 30000 });
  ...
  test('should handle errors gracefully', async ({ page }) => {
    ...
    await expect(page.locator('#latestBlockResult')).toHaveClass(/error/, { timeout: 10000 });
  ...
  test('should clear results when clear button is clicked', async ({ page }) => {
    ...
    const className = await page.locator('#latestBlockResult').getAttribute('class');
